...
import { WEAPON_CATEGORIES, WEAPON_GROUPS, weapons, weaponsById } from '../data/weapons'
import galleryManifest from '../data/weaponGalleryManifest.json'
...
const GROUP_LABELS = {
  'Melee': t('weapons.groupMelee'),
  'Handguns': t('weapons.groupHandguns'),
  'Assault Rifles': t('weapons.groupAssaultRifles'),
  'Sniper Rifles': t('weapons.groupSniperRifles'),
  'Shotguns': t('weapons.groupShotguns'),
  'Pneumatic Weapons': t('weapons.groupPneumatic'),
  'Heavy Weapons': t('weapons.groupHeavy'),
  'Throwables & Explosives': t('weapons.groupThrowables'),
  'Emplacement Weapons': t('weapons.groupEmplacement'),
}

const CATEGORY_LABELS = {
  melee: t('weapons.groupMelee'),
  handguns: t('weapons.groupHandguns'),
  assaultRifles: t('weapons.groupAssaultRifles'),
  sniperRifles: t('weapons.groupSniperRifles'),
  shotguns: t('weapons.groupShotguns'),
  pneumatic: t('weapons.groupPneumatic'),
  heavy: t('weapons.groupHeavy'),
  throwables: t('weapons.groupThrowables'),
  emplacement: t('weapons.groupEmplacement'),
}

const activeId = computed(() => route.params.id ?? '')
const activeItem = computed(() => (activeId.value ? weaponsById[activeId.value] : null))
...
const categoryNav = computed(() =>
  WEAPON_CATEGORIES.map((cat) => ({
    ...
    items: cat.ids.map((id) => weaponsById[id]).filter(Boolean),
  })),
)
...
<template>
  <section class="weapons-view">
    <!-- Detail -->
    <template v-if="activeItem">
      <button class="back-link" type="button" @click="backToList">{{ t('weapons.backToList') }}</button>

      <article class="mx-panel weapons-detail">
        <header class="detail-header">
          ...
          <p v-if="activeItem.appearances" class="detail-appearances">
            <strong>{{ t('weapons.appearances') }}:</strong> {{ activeItem.appearances }}
          </p>
        </header>
    ...
    <template v-else>
      <header class="mx-panel view-header">
        <span class="mx-tag">{{ t('weapons.tag') }}</span>
        <h1>{{ t('weapons.title') }}</h1>
        <p>{{ t('weapons.description') }}</p>
      </header>
      ...
      <div v-for="group in grouped" :key="group.group" class="cat-block">
        ...
        <ul class="weapons-grid">
          <li v-for="e in group.items" :key="e.id">
            <button class="weapons-card mx-panel" type="button" @click="openItem(e.id)">
              <img
                v-if="e.image && !brokenImages.has(e.id)"
                class="weapons-portrait"
                :src="e.image"
                :alt="e.title"
                loading="lazy"
                referrerpolicy="no-referrer"
              ...
              />
              <span v-else class="weapons-portrait placeholder" aria-hidden="true">🔫</span>
              <span class="weapons-meta">
                <span class="weapons-name">{{ e.title }}</span>
                <span v-if="e.brief" class="weapons-brief">{{ e.brief }}</span>
              </span>
            </button>
          </li>
        </ul>
      ...
      <div v-if="!grouped.length" class="mx-panel empty-state">
        ...
        <h2>{{ t('weapons.noMatches') }}</h2>
        <p>{{ t('weapons.noMatchesText') }} "{{ search }}".</p>
      </div>

      <!-- Equipment navbox -->
      <div class="mx-panel weapons-navbox">
        <h2 class="navbox-title">{{ t('weapons.navboxTitle') }}</h2>
        <div v-for="cat in categoryNav" :key="cat.label" class="navbox-row">
          ...
          <span class="navbox-items">
            <template v-for="(item, i) in cat.items" :key="item.id">
              ...
              <RouterLink :to="`/weapons/${item.id}`" class="navbox-link">{{ item.title }}</RouterLink>
            </template>
          </span>
        </div>
      </div>
