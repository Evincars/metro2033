...
import { EQUIPMENT_CATEGORIES, EQUIPMENT_GROUPS, equipment, equipmentById } from '../data/equipment'
import galleryManifest from '../data/equipmentGalleryManifest.json'
...
const GROUP_LABELS = {
  'Drugs & Consumables': t('equipment.groupDrugsConsumables'),
  'Suit & Armor': t('equipment.groupSuitArmor'),
  'Tools & Gadgets': t('equipment.groupToolsGadgets'),
  'Throwables & Explosives': t('equipment.groupThrowables'),
  'Other': t('equipment.groupOther'),
}

const CATEGORY_LABELS = {
  drugsConsumables: t('equipment.groupDrugsConsumables'),
  suitArmor: t('equipment.groupSuitArmor'),
  toolsGadgets: t('equipment.groupToolsGadgets'),
  throwables: t('equipment.groupThrowables'),
  other: t('equipment.groupOther'),
}

const activeId = computed(() => route.params.id ?? '')
const activeItem = computed(() => (activeId.value ? equipmentById[activeId.value] : null))
...
const filtered = computed(() =>
  search.value ? equipment.filter((e) => fuzzyMatch(search.value, e.title)) : equipment,
)
...
const categoryNav = computed(() =>
  EQUIPMENT_CATEGORIES.map((cat) => ({
    ...
    items: cat.ids.map((id) => equipmentById[id]).filter(Boolean),
  })),
)
...
<template>
  <section class="equipment-view">
    ...
    <template v-if="activeItem">
      <button class="back-link" type="button" @click="backToList">{{ t('equipment.backToList') }}</button>

      <article class="mx-panel equipment-detail">
        <header class="detail-header">
          ...
          <p v-if="activeItem.appearances" class="detail-appearances">
            <strong>{{ t('equipment.appearances') }}:</strong> {{ activeItem.appearances }}
          </p>
        </header>
    ...
    <template v-else>
      <header class="mx-panel view-header">
        <span class="mx-tag">{{ t('equipment.tag') }}</span>
      ...
      <div v-for="group in grouped" :key="group.group" class="cat-block">
        ...
        <ul class="equipment-grid">
          <li v-for="e in group.items" :key="e.id">
            <button class="equipment-card mx-panel" type="button" @click="openItem(e.id)">
              <img
                v-if="e.image && !brokenImages.has(e.id)"
                class="equipment-portrait"
                :src="e.image"
                :alt="e.title"
                loading="lazy"
                referrerpolicy="no-referrer"
              ...
              />
              <span v-else class="equipment-portrait placeholder" aria-hidden="true">⚙</span>
              <span class="equipment-meta">
                <span class="equipment-name">{{ e.title }}</span>
                <span v-if="e.brief" class="equipment-brief">{{ e.brief }}</span>
              </span>
            </button>
          </li>
        </ul>
      ...
      <div v-if="!grouped.length" class="mx-panel empty-state">
        ...
        <h2>{{ t('equipment.noMatches') }}</h2>
        <p>{{ t('equipment.noMatchesText') }} "{{ search }}".</p>
      </div>

      <!-- Equipment navbox -->
      <div class="mx-panel equipment-navbox">
        <h2 class="navbox-title">{{ t('equipment.navboxTitle') }}</h2>
        <div v-for="cat in categoryNav" :key="cat.label" class="navbox-row">
          ...
          <span class="navbox-items">
            <template v-for="(item, i) in cat.items" :key="item.id">
              ...
              <RouterLink :to="`/equipment/${item.id}`" class="navbox-link">{{ item.title }}</RouterLink>
            </template>
          </span>
        </div>
      </div>
...
.equipment-portrait.placeholder {
  display: flex;
  align-items: center;
