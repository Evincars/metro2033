---
id: pionerskaya
title: "Пионерская"
line: 4
lineName: "Филёвская линия"
lineNameEn: "Filyovskaya Line"
opened: "13 октября 1961"
architects: "Р. И. Погребной, В. А. Черёмин"
image: "/real-metro-imgs/pionerskaya.jpg"
gallery: "https://www.metrowalks.com/f/msk/pioner/0.jpg|https://www.metrowalks.com/f/msk/pioner/220820-1135954.jpg|https://www.metrowalks.com/f/msk/pioner/220820-1135938.jpg|https://www.metrowalks.com/f/msk/pioner/220820-1135941.jpg"
source: "https://www.metrowalks.com/ru/moscow/filyovskaya/pionerskaya"
brief: "Пионерская — наземная станция, открытая, с островной платформой."
---

Пионерская — наземная станция, открытая, с островной платформой.

Станция находится в западной части города в районе Фили-Давыдково у перекрёстка Мазиловской и Малой Филёвской улиц.

Происхождение названия: в честь Всесоюзной пионерской организации (название не привязано к местности).

Дата открытия — 13 октября 1961.

Архитекторы станции — Р. И. Погребной, В. А. Черёмин.

Станция построена по типовому проекту с островной платформой, одинаковому с соседними станциями. Над платформой устроен навес, но зона над путями
      не полностью крытая. В центральной части станции имеются путевые стены (основание виадука Мазиловской улицы), изначально окрашенные жёлтой краской.
      Платформы были покрыты асфальтом. Колонны облицованы белым мрамором.
