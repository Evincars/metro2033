...
brief: "The Trench Knife is a metro-made weapon that is carried by Artyom at all times throughout his travels."
wiki: Trench_Knife
...
The knife has two attack styles, left click/left trigger is a fast attack, while right click/right trigger is a slower bash. The slower bash is the style Artyom uses when a mutant is on top of him. If the primary attack is tapped rapidly, Artyom will perform three slashes in quick succession. The secondary attack (using the brass knuckles) can instantly kill a human with one blow to the head on any difficulty, even if they are wearing a helmet or [gas mask](https://metrovideogame.fandom.com/wiki/gas_mask).

During quick-time events throughout the game, Artyom uses his knife for a multitude of tasks, from stabbing or bashing mutants in the head and neck who have pounced him, to breaking ice off of cables to allow him to continue climbing.

In *[Metro: Last Light](https://metrovideogame.fandom.com/wiki/Metro:_Last_Light)*, the knife is much easier to use and quite valuable to a stealthy playthrough. When sneaking up on a human enemy, getting close enough to them results in the sound of Artyom unsheathing the knife, as well as an on-screen prompt in [Normal gameplay modes](https://metrovideogame.fandom.com/wiki/Difficulties#Standard_Difficulties), allowing him to select between a silent assassination (via pressing the melee attack button), or a silent knock-out (via pressing the interact button). These attacks are also used in [Ranger mode](https://metrovideogame.fandom.com/wiki/Difficulties#Ranger_Difficulties_(DLC)); the unsheathing sound remains to notify players that they are close enough for a takedown, though the on-screen prompt is removed. One exception is in the "[Sniper Team](https://metrovideogame.fandom.com/wiki/Sniper_Team_(Faction_Pack_DLC_Level))" mission, where the player character can only kill. Additionally, in the mission that stealth is not necessary, the sneak attack of the Trench Knife is normally useless; the non-lethal attack does has its merits however, it counts as complete the level stealthy and in some levels the player character will still earn positive [moral point](https://metrovideogame.fandom.com/wiki/Moral_Points) if all the attacks are non-lethal.

The animations for killing/knocking out an enemy stealthily are different, depending on which angle the player character approaches and if the enemy is standing or sitting down, and if the enemy has been alerted to their presence. For example, if coming up from behind an enemy (who has not detected the player character), if one chooses to kill him, the character will grab his head, slit his throat, and then slowly put the now dead hostile on the ground. If the player character knocks him out, they will grab the enemy by the right shoulder, turn him around, and punch him in the face with the duster knuckles. Another example, if approaching from the left side of an enemy, he will either be stabbed in the back of the throat, or he will be turned around and be punched in the face with the knuckles. Sometimes, a player character can sneak up on an enemy and not see the prompt to either kill him or knock him out. If the player presses the melee button, he will simply stab him. The player will not have to worry about being spotted if this happens. The kill may seem loud, but it will not attract the attention of other enemies (unless if the player character is spotted).

Both of these attacks are almost impossible for enemies to detect if executed out of sight, allowing for a swift, effortless take down and easy conservation of ammunition. Sufficiently sneaky players characters can make their way through entire stations of human enemies using nothing but their trench and throwing knives, resulting in a tidy profit.
...
- In *[Metro 2033 Redux](https://metrovideogame.fandom.com/wiki/Metro_2033_Redux)* and the sequels, Artyom's knife doesn't occupy an inventory slot, instead being used at the push of a button.
